...
export default async function DashboardPage() {
  ...
  const supportedCountrySet = new Set([...SUPPORTED_COUNTRIES, "India"]);

  let areas = (areasData?.areas || []).filter((area) =>
    supportedCountrySet.has(area.strArea)
  );

  // Ensure India is present
  if (!areas.some((a) => a.strArea === "India")) {
    areas.push({ strArea: "India" });
  }

  // Ensure Netherlands is present
  if (!areas.some((a) => a.strArea === "Netherlands")) {
    areas.push({ strArea: "Netherlands" });
  }

  // Ensure France is present
  if (!areas.some((a) => a.strArea === "France")) {
    areas.push({ strArea: "France" });
  }

  // Ensure Norway is present
  if (!areas.some((a) => a.strArea === "Norway")) {
    areas.push({ strArea: "Norway" });
  }

  // Ensure Venezuela is present
  if (!areas.some((a) => a.strArea === "Venezuela")) {
    areas.push({ strArea: "Venezuela" });
  }

  // ✅ Sort alphabetically
  areas = areas.sort((a, b) => a.strArea.localeCompare(b.strArea));
