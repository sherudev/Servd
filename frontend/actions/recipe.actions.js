    ...
    if (searchResponse.ok) {
      ...
      if (searchData.data && searchData.data.length > 0) {
        ...
        const savedRecipeResponse = await fetch(
          `${STRAPI_URL}/api/saved-recipes?filters[user][id][$eq]=${user.id}&populate=recipe`,
          {
            headers: {
              Authorization: `Bearer ${STRAPI_API_TOKEN}`,
            },
        ...
        if (savedRecipeResponse.ok) {
          ...
          isSaved = savedData.data.some(
            (item) => item.recipe?.id === searchData.data[0].id
          );
        }
...
export async function removeRecipeFromCollection(formData) {
  try {
    ...
    console.log("👉 recipeId received:", recipeId);

    // Find saved recipe relation
    const searchResponse = await fetch(
      `${STRAPI_URL}/api/saved-recipes?filters[user][id][$eq]=${user.id}&filters[recipe][$eq]=${recipeId}`,
      {
        headers: {
          Authorization: `Bearer ${STRAPI_API_TOKEN}`,
        },
    ...
    for (const item of searchData.data) {
      await fetch(`${STRAPI_URL}/api/saved-recipes/${item.id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${STRAPI_API_TOKEN}`,
        },
